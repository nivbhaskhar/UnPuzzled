...
def left_right_adj_score(P, Q, R, S, model_name, model,device):   
    ...
    with torch.no_grad():
            ...
            new_input_torchtensor =  new_input_torchtensor.to(device)
            if model_name=="AdjacencyClassifier_NoML":
                score = model.negative_distance_score(new_input_torchtensor).cpu().numpy()
                return score[0]
            elif model_name=="RandomScorer":
                return random.random()
            else:
                score = model(new_input_torchtensor).cpu().numpy()
                return score[0,1]
                


def compute_and_memoize_score(information_dict, shuffled_puzzle_pieces_np, P, Q, R, S, model_name, model,device):
...
class PuzzleBoard:
    ...
    def __init__(self, rows, cols, information_dict, top_left_piece_new_label, 
                 top_left_piece_orientation, shuffled_puzzle_pieces_np, model_name, model,device):
        self.rows = rows
        self.cols = cols
                    ...
                    for i in range(4):
                        if self.state[open_slot][i][0] is not None:
                            ...
                            current_score = compute_and_memoize_score(self.information_dict,
                                                                      ...
                                                                      NR_,R_,self.model_name,self.model,self.device)
...
def solve_puzzle(rows, cols, top_left_piece_new_label, top_left_piece_orientation,
                 shuffled_puzzle_pieces_np, puzzle_square_piece_dim, model_name, model,device,
    ...
    my_board = PuzzleBoard(rows, cols, information_dict, top_left_piece_new_label, 
                           ...
                           shuffled_puzzle_pieces_np,model_name, model,device)
...
def solve_example(file_name, puzzle_square_piece_dim,model_name, model, device,show_solving_progress=False,input_display=False):
